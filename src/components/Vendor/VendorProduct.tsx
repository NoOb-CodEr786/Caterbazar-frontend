...
export default function VendorDetailsPage({ vendor, setupImages }: VendorDetailsPageProps) {
  ...
  const currentImage = setupImages.length > 0 
    ? setupImages[currentImageIndex].url 
    : (vendor.profilePhoto || 'https://developers.elementor.com/docs/assets/img/elementor-placeholder-image.png');
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="py-2 hidden sm:block">
        <div className="container mx-auto px-4 sm:px-6 py-2 sm:py-3">
          <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm overflow-x-auto hide-scrollbar">
            ...
            <a href="/vendors" className="text-orange-500 hover:text-orange-600 whitespace-nowrap">Vendors</a>
            <span className="text-gray-400">/</span>
            <a href={`/vendors?state=${vendor.address.state}`} className="text-orange-500 hover:text-orange-600 whitespace-nowrap">{vendor.address.state}</a>
            <span className="text-gray-400">/</span>
            <span className="text-gray-700 whitespace-nowrap">{vendor.businessRegistrationId?.brandName || vendor.userId.fullName}</span>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-4 sm:space-y-6">
            ...
            <div className="bg-white rounded-lg sm:rounded-xl overflow-hidden shadow-md group">
              <div className="relative aspect-video sm:aspect-[16/10] lg:aspect-[16/9]">
                <img
                  src={currentImage}
                  alt={vendor.businessRegistrationId?.brandName || vendor.userId.fullName}
                ...
                {setupImages.length > 1 && (
                  <>
                    <button
                      onClick={() => setCurrentImageIndex(prev => prev === 0 ? setupImages.length - 1 : prev - 1)}
                    ...
                    <button
                      onClick={() => setCurrentImageIndex(prev => prev === setupImages.length - 1 ? 0 : prev + 1)}
              ...
              <div className="flex items-start justify-between gap-3 mb-3 sm:mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1 sm:mb-2">
                    <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">{vendor.businessRegistrationId?.brandName || vendor.userId.fullName}</h1>
                    {vendor.isCaterbazarChoice && (
                      <span className="bg-orange-500 text-white text-xs px-2 py-1 rounded-full font-medium">CB Choice</span>
                    )}
                  </div>
                  <p className="text-xs sm:text-sm lg:text-base text-gray-600 mb-2 sm:mb-3">
                    {formatText(vendor.capacity.vendorCategory)} • {cuisineDisplay}
                  </p>
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                    <div className="flex items-center gap-1">
                      <Star className="w-4 h-4 sm:w-5 sm:h-5 fill-yellow-400 text-yellow-400" />
                      <span className="text-sm sm:text-base font-semibold">{vendor.stats.averageRating.toFixed(1)}</span>
                      <span className="text-xs sm:text-sm text-gray-600">• {vendor.stats.totalReviews} reviews</span>
                    </div>
                    <div className="flex items-center gap-1 text-gray-600">
                      <MapPin className="w-3 h-3 sm:w-4 sm:h-4" />
                      <span className="text-xs sm:text-sm">{vendor.address.locality}, {vendor.address.state}</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-lg sm:rounded-2xl p-4 sm:p-6 shadow-lg">
              <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 mb-3 sm:mb-4">About {vendor.businessRegistrationId?.brandName || vendor.userId.fullName}</h2>
              <div className="space-y-3 sm:space-y-4 text-xs sm:text-sm lg:text-base text-gray-700">
                <p>{vendor.bio || 'Professional catering service dedicated to making your events memorable with exceptional food and service.'}</p>
                <p>Established in {vendor.businessInfo.yearOfEstablishment}, with {vendor.businessInfo.yearsInBusiness}+ years of experience in the catering industry.</p>
                {vendor.pricing.servicesSpecialization.length > 0 && (
                  <p>Specializing in: {vendor.pricing.servicesSpecialization.map(formatText).join(', ')}</p>
                )}
              </div>
              ...
              <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="flex items-center gap-3 p-3 bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{vendor.businessInfo.yearOfEstablishment}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 bg-gradient-to-br from-green-50 to-green-100 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{vendor.businessInfo.yearsInBusiness}+ Years</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{vendor.businessInfo.teamSize}</p>
                  </div>
                </div>
              </div>
              ...
              {(vendor.socialMedia?.facebookHandle || vendor.socialMedia?.instagramHandle || vendor.socialMedia?.personalWebsite) && (
                <div className="mt-6">
                  <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-3">Connect With Us</h3>
              ...
              {vendor.fssaiCertificate?.url && (
                <div className="mt-6 p-4 bg-green-50 border-2 border-green-200 rounded-lg">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h3 className="text-base font-bold text-green-900 mb-1">FSSAI Certified</h3>
                      <p className="text-sm text-green-700 mb-3">This vendor is certified by the Food Safety and Standards Authority of India</p>
                      <a
              ...
              {/* Operations Info */}
              <div className="mt-6">
                <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-3">Operations</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Languages Spoken</p>
                    <p className="text-sm font-semibold text-gray-900">{vendor.operations.languagesSpoken.map(formatText).join(', ')}</p>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Service Radius</p>
                    <p className="text-sm font-semibold text-gray-900">{vendor.operations.operationalRadius} km</p>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Advance Booking</p>
                    <p className="text-sm font-semibold text-gray-900">{vendor.operations.weeksAdvanceBooking} weeks minimum</p>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Profile Views</p>
                    <p className="text-sm font-semibold text-gray-900">{vendor.analytics?.profileViews || 0} views</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4  mb-4 sm:mb-6 mt-6 sm:mt-10">
                <div className="text-center p-3 sm:p-4 bg-orange-50 rounded-lg">
                  <h3 className="text-sm sm:text-base font-bold text-gray-900 mb-0.5 sm:mb-1">Cuisines</h3>
                  <p className="text-[10px] sm:text-xs text-gray-600">{vendor.pricing.cuisineOptions.map(formatText).slice(0, 5).join(', ')}</p>
                  {vendor.pricing.cuisineOptions.length > 5 && (
                    <p className="text-[10px] text-orange-600 mt-1">+{vendor.pricing.cuisineOptions.length - 5} more</p>
                  )}
                </div>
                <div className="text-center p-3 sm:p-4 bg-orange-50 rounded-lg">
                  <h3 className="text-sm sm:text-base font-bold text-gray-900 mb-0.5 sm:mb-1">{vendor.capacity.minGuests}-{vendor.capacity.maxGuests} Guests</h3>
                  <p className="text-[10px] sm:text-xs text-gray-600">Flexible capacity</p>
                </div>
                <div className="text-center p-3 sm:p-4 bg-orange-50 rounded-lg">
                  <h3 className="text-sm sm:text-base font-bold text-gray-900 mb-0.5 sm:mb-1">{vendor.stats.totalOrders || 0}</h3>
                  <p className="text-[10px] sm:text-xs text-gray-600">Orders Completed</p>
                </div>
                <div className="text-center p-3 sm:p-4 bg-orange-50 rounded-lg">
                  <h3 className="text-sm sm:text-base font-bold text-gray-900 mb-0.5 sm:mb-1">{vendor.analytics?.responseRate || 0}%</h3>
                  <p className="text-[10px] sm:text-xs text-gray-600">Response Rate</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-md lg:sticky lg:top-24">
              ...
              {(vendor.capacity.vendorCategory === 'full_catering' || vendor.capacity.vendorCategory === 'other') ? (
                ...
                <div>
                  ...
                  <div className="flex items-baseline gap-1 sm:gap-2">
                  <span className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">₹{vendor.pricing.nonVegPricePerPlate}</span>
                  <span className="text-[10px] sm:text-xs text-gray-600">/ Plate</span>
                  </div>
                </div>
                </div>
              ...
              <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
                ...
                <div className="flex items-center gap-2 text-xs sm:text-sm">
                  ...
                  <span className="text-gray-700">{formatText(vendor.cancellationPolicy.policyType)}</span>
                </div>
                <div className="flex items-center gap-2 text-xs sm:text-sm">
                  <div className="w-4 h-4 sm:w-5 sm:h-5 bg-green-100 rounded flex items-center justify-center shrink-0">
                    <span className="text-green-600 text-[10px] sm:text-xs">✓</span>
                  </div>
                  <span className="text-gray-700">{vendor.capacity.advanceBookingTime} advance booking</span>
                </div>
                {vendor.fssaiCertificate?.url && (
                  <div className="flex items-center gap-2 text-xs sm:text-sm">
                    <div className="w-4 h-4 sm:w-5 sm:h-5 bg-green-100 rounded flex items-center justify-center shrink-0">
                ...
                <div className="flex items-center gap-2 text-xs sm:text-sm">
                  ...
                  <span className="text-gray-700">{vendor.profileCompletionPercentage}% Profile Complete</span>
                </div>
              </div>
              ...
              <div className="space-y-2 sm:space-y-3">
                  {(() => {
                    const getWhatsAppMessage = () => {
                      const brandName = vendor.businessRegistrationId?.brandName || vendor.userId.fullName;
                      const location = `${vendor.address.locality}, ${vendor.address.state}`;
                      const category = formatText(vendor.capacity.vendorCategory);
                      const cuisines = vendor.pricing.cuisineOptions.map(formatText).join(', ');
                      ...
                      if (vendor.capacity.vendorCategory === 'full_catering' || vendor.capacity.vendorCategory === 'other') {
      ...
      <InquiryModal
        ...
        vendorName={vendor.businessRegistrationId?.brandName || vendor.userId.fullName}
      />
    </div>
  );
}